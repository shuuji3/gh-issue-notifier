...
async function run() {
    ...
    for (const repo of config.repos) {
        try {
            ...
            if (issues.length > 0) {
                await sendNotification(repo, issues, isDebug, dateLimit);
...
async function getIssues(repo: string, isDebug: boolean, dateLimit: string) {
    ...
    const issues: { number: number; title: string; url: string; createdAt: string }[] = JSON.parse(stdout);

    const filteredIssues = isDebug ? issues : issues.filter(i => new Date(i.createdAt) < new Date(dateLimit));
    return filteredIssues.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
}

async function sendNotification(repo: string, issues: any[], isDebug: boolean, dateLimit: string) {
